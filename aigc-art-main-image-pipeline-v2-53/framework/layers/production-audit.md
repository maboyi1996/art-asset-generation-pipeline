...
在提示词生成前检查前序结构化结果是否完整、一致、可进入生产，尤其检查应生产资产是否已经进入 `global-task-draft`。
...
只做一致性审计和问题标记，不重新抽取资产、不重新拆状态、不重新评级、不补写资产任务、不重写提示词。发现 B/C 可生产人物只有去向、没有任务时，必须返回 blocking 问题并指回 `asset-registry.md`。
