...
V2.60 将 V2.56 中过重的 `asset-registry.md` 拆回单一职责：注册层只负责资产 ID、状态 ID 和别名归并；人物去向、任务草稿和分集映射分别由独立层级生成。
...
flowchart TD
  ...
  SC --> AE["asset-extraction.md<br/>输出: asset-candidate-list<br/>polluted-label-cleanup-ledger"]
...
| 模块 | 顺序 | 作用 |
|---|---|---|
| 剧本解析与资产识别 | `script-intake -> scene-coverage -> asset-extraction -> state-variant -> asset-rating -> asset-registry -> candidate-disposition -> task-manifest-generation -> episode-usage-map` | 建立场次事实、候选资产、污染清洗记录、状态线索去向、评级、资产注册表、候选去向、任务草稿和分集映射草稿。 |
...
| 剧本解析与资产识别 | `asset-extraction.md` | `scene-coverage-table` | `资产抽取边界规则.md`、`污染标签清洗记录规则.md` | `asset-candidate-list`、`polluted-label-cleanup-ledger` |
...
| 资产生产与提示词生成 | `production-audit.md` | `scene-coverage-table`、`asset-candidate-list`、`state-variant-result`、`state-clue-disposition-ledger`、`asset-rating-result`、`global-asset-registry`、`alias-resolution-map`、`character-disposition-ledger`、`generic-template-coverage-ledger`、`polluted-label-cleanup-ledger`、`global-task-draft`、`episode-usage-map-draft`、`character-anchor-result` | `生产审计规则.md` | `production-audit-result` |
...
| 输出交付与质检 | `output-contracts.md` | `script-intake-result`、`global-asset-registry`、`alias-resolution-map`、`state-variant-result`、`state-clue-disposition-ledger`、`character-disposition-ledger`、`generic-template-coverage-ledger`、`polluted-label-cleanup-ledger`、`prompted-production-task-table`、`episode-usage-map-draft`、`production-audit-result`、`character-anchor-result` | `任务类型字段审核规则.md`、`分集映射规则.md`、`机器长表模板.md`、`人审短表模板.md` | `main-image-production-table.md`、`main-image-review-table.md` |
...
- `scene-coverage-table` 被 `asset-extraction.md`、`state-variant.md`、`asset-rating.md`、`episode-usage-map.md`、`production-audit.md` 接住。
