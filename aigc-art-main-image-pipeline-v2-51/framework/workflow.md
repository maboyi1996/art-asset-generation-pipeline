...
| 模块 | 顺序 | 作用 |
|---|---|---|
| 剧本解析与资产识别 | `script-intake -> scene-coverage -> asset-extraction -> state-variant -> asset-rating -> asset-registry` | 建立事实、候选资产、状态、评级、任务草稿、人物候选去向台账和分集映射草稿。 |
...
| 剧本解析与资产识别 | `asset-registry.md` | `asset-candidate-list`、`state-variant-result`、`asset-rating-result`、`scene-coverage-table` | `任务类型字段审核规则.md`、`分集映射规则.md`、`人物候选去向闭环规则.md` | `global-asset-registry`、`global-task-draft`、`character-disposition-ledger`、`episode-usage-map-draft` |
