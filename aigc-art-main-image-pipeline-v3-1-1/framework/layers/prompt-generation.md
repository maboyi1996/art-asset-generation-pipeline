...
V3.1.1 的本层不得直接输出整段最终 prompt。模型只能把剧本证据、注册结果、状态结果、锚点结果和审计结论转写为结构化槽位。中文 core prompt 和中文状态编辑 core instruction 由 `scripts/render_core_prompts.py` 确定性渲染。

填写槽位前必须读取 `core-prompt-slots规则.md` 中“槽位值可画视觉细节准入规则”和人物、场景、道具三类样例。样例用于约束合格细节密度和可画程度，不是固定文案；不得只按空白模板的字段名输出泛泛描述。
...
本层只读取已存在的 `task_id`、`base_asset_id`、`state_asset_id`、`priority_level`、`task_type`、`review_status`、状态、锚点和证据字段，并按 `core-prompt-slots规则.md` 填入槽位。`slot_value` 必须把证据转写成可画视觉细节；不得把称谓关系、身份推理、抽象评价或“与身份气质一致”一类文字当作槽位值。
