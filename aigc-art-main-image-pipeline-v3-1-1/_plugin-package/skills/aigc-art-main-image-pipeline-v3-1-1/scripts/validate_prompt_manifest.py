#!/usr/bin/env python3
"""Validate Seedance prompt-manifest.json with V3.1.1 hard checks."""
...
def validate_core_prompt(value: Any, prefix: str, errors: List[str]) -> None:
    expect(isinstance(value, str) and bool(value), f"{prefix}.imagePrompt is required", errors)
    if not isinstance(value, str):
        return
    expect(contains_cjk(value), f"{prefix}.imagePrompt must be Chinese core prompt text", errors)
    lowered = value.lower()
    for marker in FORBIDDEN_PROMPT_MARKERS:
        expect(marker.lower() not in lowered, f"{prefix}.imagePrompt contains forbidden marker: {marker}", errors)
...
def validate_manifest(data: Any) -> List[str]:
    ...
    for asset_index, asset in enumerate(assets):
        ...
        for state_index, state in enumerate(appearances):
            ...
            validate_core_prompt(state.get("imagePrompt"), state_prefix, errors)
            ...
            if isinstance(generation, dict):
                ...
                if method in {"text_to_image", "image_text_edit"}:
                    ...
                    expect(isinstance(negative_prompt, str) and bool(negative_prompt), f"{state_prefix}.negativePrompt is required", errors)
                    if isinstance(negative_prompt, str):
                        expect(contains_cjk(negative_prompt), f"{state_prefix}.negativePrompt must be Chinese", errors)
