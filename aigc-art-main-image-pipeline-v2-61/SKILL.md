---
name: aigc-art-main-image-pipeline-v2-61
description: "Use this skill when Codex needs to read Chinese screenplays, novel drafts, episodic drama scripts, or project script files and produce story-faithful AIGC art main-image assets for characters, scenes, and props. V2.61 adds lossless pre-rating evidence ledgers for characters, scenes, and props; ratings use those ledgers as primary input while allowing source-text verification; production filtering happens only after S/A/B/C rating."
---

# AIGC Art Main Image Pipeline V2.61
...
|---|---|---|
| Asset identification | `script-intake` through `episode-usage-map` | Build scene facts, lossless pre-rating evidence ledgers, candidate assets, polluted-label cleanup, state variants, state-clue disposition, continuity ratings, pure asset registry, candidate disposition, task drafts, and episode usage mapping drafts. |
...
- `character-evidence-ledger.md`、`scene-evidence-ledger.md`、`prop-evidence-ledger.md` are lossless pre-rating evidence ledgers. They do not rate, register, generate tasks, or decide production value.
- Ratings use the matching evidence ledger as the primary input, `scene-coverage-table` as context, and source text only to verify evidence. Ratings must not rescan PDFs, recut scenes, or silently invent candidates.
- `asset-candidate-list` is a candidate handoff result, not the only source of rating truth.
- If a rating layer finds visible evidence missing from an evidence ledger or candidate list, it must block and point back to the evidence or extraction layer instead of silently patching downstream.
- `asset-extraction.md` admits candidate assets from the three evidence ledgers and outputs `polluted-label-cleanup-ledger`.
...
- Polluted labels such as `皇帝点头`、`棠宁喃喃`、`皇后拍桌`、`二夫人三夫人一起` may be recorded only in evidence ledger raw fields, `polluted-label-cleanup-ledger.raw_label`, or source evidence. They must not enter `asset-candidate-list`, `character-disposition-ledger`, asset names, state names, task names, or episode mappings.
...
`main-image-production-table.md` must preserve all production tasks, states, prompts, edit instructions, review statuses, evidence, episode usage references, the three pre-rating evidence ledgers, the global asset registry, alias map, state index, state-clue disposition table, character candidate disposition audit section, generic template coverage section, polluted-label cleanup record section, and production audit summary.
