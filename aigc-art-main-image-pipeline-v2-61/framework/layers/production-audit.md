...
在提示词生成前检查前序结构化结果是否完整、一致、可进入生产，尤其检查三类评级前证据账本、事实覆盖、候选去向、污染清洗、状态主体、任务草稿、分集映射和锚点依赖是否断链。
...
只做一致性审计和问题标记，不重新抽取资产、不重新拆状态、不重新评级、不补写资产任务、不重写提示词。发现断链时必须指回具体拥有层：证据账本漏项回对应 evidence layer，污染清洗回 `asset-extraction.md`，状态主体回 `state-variant.md`，候选去向回 `candidate-disposition.md`，任务缺失回 `task-manifest-generation.md`，映射缺引用回 `episode-usage-map.md`。
