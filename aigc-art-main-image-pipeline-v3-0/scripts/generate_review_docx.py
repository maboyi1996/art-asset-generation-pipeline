#!/usr/bin/env python3
"""Generate main-image-review-table.docx from main-image-production-table.md.
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate main-image-review-table.docx from main-image-production-table.md")
    parser.add_argument("--production", required=True, type=Path, help="Path to main-image-production-table.md")
    parser.add_argument("--output", default="main-image-review-table.docx", type=Path, help="Output DOCX path")
    args = parser.parse_args(argv)

    try:
        episodes = derive_review_rows(args.production)
        write_docx(args.output, build_document_xml(episodes))
