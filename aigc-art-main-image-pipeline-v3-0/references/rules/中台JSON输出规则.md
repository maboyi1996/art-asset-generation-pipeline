...
> 如何把 V3.0 机器长表中的已确认角色、场景、道具主图任务转换为中台可导入的 JSON 结构？
...
| `source.skillId` | 固定为 `aigc-art-main-image-pipeline-v3-0`。 |
| `source.skillVersion` | 固定为 `3.0`。 |
...
- 不改变 `main-image-production-table.md` 或 `main-image-review-table.docx` 的内容来迎合 JSON。
