---
name: aigc-art-main-image-pipeline-v2-1
description: "Use this skill when the user wants Codex to read Chinese screenplays, novel drafts, episodic drama scripts, or project script files and produce story-faithful AIGC art main-image assets for characters, scenes, and props. V2.1 uses a hybrid workflow: identify assets by episode/scene, produce prompts by global asset state, then deliver two MD outputs by default. It hard-separates the complete machine/source-of-truth production table from the derived human review table and guards against summary-like outputs, blocked-state loss, B/C omission, and coverage regression."
---

# AIGC Art Main Image Pipeline V2.1

This skill converts scripts into production-ready **main image** tasks for art review.

V2.1 uses a hybrid architecture:
...
15. `main-image-production-table.md` is allowed to be long and wide. Do not summarize, sample, collapse, or shorten the machine table to make it easier for humans to inspect.
16. `blocked`, `manual_review`, `pending_reference`, or `blocked_identity_anchor_required` status never removes a state from the machine table or episode mapping. Keep the row, stable IDs, evidence, blocking reason, and usage rows visible.
17. B/C assets may be `record_only`, `reuse_existing`, `manual_review`, or omitted from prompt generation, but production-relevant or episode-mapped B/C assets must not disappear from the registry, machine table, or episode usage mapping.
18. If final task counts, asset-type counts, `image_text_edit` rows, or episode mapping rows are sharply lower than a prior run, baseline, or the density implied by the script, stop and report a coverage-regression risk instead of silently delivering a compressed result.
19. Separate priority from handling. Use `priority_level: S/A/B/C/D` for importance, and `production_handling: main_task/deferred_task/record_only_mapping/exclude_with_reason/manual_review` for destination. Never let a non-S/A priority silently delete an asset.
20. S must be `main_task`; A defaults to `main_task` unless a downgrade reason is explicit; B must be deferred, mapped, or excluded with reason; C must be mapped when physical appearance affects episode understanding or excluded with reason; D is pure background/mentioned-only and goes to the exclusion log by default.
...
Completeness rule: this file is the source of truth for AI models and downstream parsing. It must preserve all production-relevant character, scene, and prop states, including blocked/manual-review states, record-only mapped states, reused states, and B/C states with episode usage. The human review file must never be used as the target shape for this table.
