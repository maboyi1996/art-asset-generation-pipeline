...
This layer writes prop analysis and main image prompts for global asset-state tasks. It must use the PDF-verified prop templates.
...
This layer works from the Global Asset State Task Map. Do not create prop prompts directly from episode tables.
...
- `used_in_episodes`
- `used_in_scenes`
- `reuse_from_task_id` when this prop state is reused
...
- each prop production row has `task_id` and `task_type`
- reused prop states are marked `reuse_existing` instead of regenerated
