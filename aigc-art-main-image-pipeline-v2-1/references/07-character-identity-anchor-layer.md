...
1. Build the `Key Character Identity Anchor Table` from the global character registry before any character state task is produced.
...
6. Every later global character state for that same base character must default to `image_edit_from_identity_anchor` unless it is visually identical and can use `reuse_existing`.
7. State prompts must reuse the approved reference image and add only the state delta.

If no approved identity image exists yet, mark later state tasks as `blocked_identity_anchor_required` or `manual_review`, not as independent text-to-image tasks. This status must not delete or merge the state: keep the state in the Global Asset State Task Table with its `state_asset_id`, `task_id`, evidence, blocking reason, and episode usage mapping.

## Master List Requirements

The Key Character Identity Anchor Table / Main Character Master List is not only an art-review summary. It is the identity source of truth for downstream character state generation and must exist before character state tasks are generated.
...
- `state_asset_id`
- `task_id`
- `task_type`: usually `image_text_edit` or `reuse_existing`
- `identity_anchor_task_id`
...
`generation_mode` describes character identity handling. `task_type` describes the production task category used by the Global Asset State Task Table.

Use:
- `task_type: text_to_image` with `generation_mode: text_to_image_identity_anchor` for the first key-character identity portrait.
- `task_type: image_text_edit` with `generation_mode: image_edit_from_identity_anchor` for normal later state edits.
- `task_type: image_text_edit` with `generation_mode: image_edit_identity_sensitive_state` for script-supported identity-sensitive visible changes.
- `task_type: reuse_existing` when a later episode uses a visually identical already generated state.
- `task_type: blocked` when the required identity reference image is missing or not approved.
- `task_type: manual_review` when identity continuity or state evidence needs human confirmation.

For blocked identity-dependent states, use `task_type: blocked` and a status/reason such as `blocked_identity_anchor_required`. Do not remove the state from the machine table, and do not hide it from episode mapping.
...
Character rows in the Global Asset State Task Table must include:
...
For the identity anchor task itself, use `task_type: text_to_image`, `generation_mode: text_to_image_identity_anchor`, and `identity_reference_image: pending` until an approved image path/URL/task reference is available.
...
- a later state is generated as a new `text_to_image` task when it should be `image_text_edit` or `reuse_existing`
