...
Completeness guard:
- `main-image-production-table.md` is allowed to be long and wide. Do not shorten it for human readability.
- It must include all production-relevant and episode-mapped character, scene, and prop states from the Global Asset State Task Map.
- Blocked/manual-review/pending-reference rows still count as global tasks and must remain joinable from episode usage rows.
- B/C rows that are production-relevant or episode-mapped must remain visible as `record_only`, `reuse_existing`, `manual_review`, or another explicit non-prompt status.
- If a prior/baseline run exists or the script density implies far more assets, compare counts. A sharp drop in total global tasks, asset-type counts, `image_text_edit` rows, or episode mapping rows is a failed output, not a valid concise result.
...
Every production-relevant character, scene, and prop state must appear here once. Every episode-mapped character, scene, and prop state must also appear here, even if it is B/C, blocked, manual-review, pending-reference, record-only, or reused. Do not repeat the same `state_asset_id` as multiple independent generation tasks unless the split is explicit and justified.
...
Blocked/manual-review rows are not optional. They must keep `task_id`, `state_asset_id`, evidence, first-needed episode/scene, blocking/review reason, and episode usage joins so the omission is visible to downstream users.
...
- every `priority_level: S` state uses `production_handling: main_task`
- every `priority_level: A` state uses `main_task` unless a downgrade reason is explicit
- every `priority_level: B` state has `deferred_task`, `record_only_mapping`, or `exclude_with_reason`
- every `priority_level: C` state has `record_only_mapping` or `exclude_with_reason`, with physically relevant appearances mapped
- every `priority_level: D` exclusion has an explicit reason
- every episode-mapped B/C, blocked, manual-review, pending-reference, record-only, or reused state has a stable `task_id`
...
- coverage counts have been compared against prior/baseline output when available, or against script density when no baseline exists; sharp unexplained drops are reported instead of accepted
