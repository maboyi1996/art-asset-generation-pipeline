...
- summary-like compression or unexplained coverage regression
...
- `priority_level`, `production_handling`, and derived `needs_main_image` are explicit
...
- B/C, blocked, manual-review, pending-reference, record-only, and reused states remain visible when they are production-relevant or episode-mapped
...
- an episode-mapped state disappears because it is B/C, blocked, manual-review, pending-reference, record-only, or reused
...
Pass criteria:
- every asset has `priority_level: S/A/B/C/D`
- every asset has `production_handling`
- every rating and handling decision has a reason
- every S item has `production_handling: main_task`
- every A item is `main_task` unless a downgrade reason is explicit
- every B item has `deferred_task`, `record_only_mapping`, or `exclude_with_reason`
- every C item has `record_only_mapping` or `exclude_with_reason`, and physical appearances that affect scene understanding are mapped
- every D item is excluded only with a reason
- B/C/D exceptions are explicit
- B/C assets with episode usage are retained as mapped record-only/reuse/manual-review rows when they do not get prompts

Fail if:
- rating is unexplained
- `priority_level` and `production_handling` are collapsed into one ambiguous field
- core asset is B/C without reason
- an S asset is not `main_task`
- an A asset is not `main_task` and has no downgrade reason
- a B asset silently disappears instead of being deferred, mapped, or excluded with reason
- a C physical appearance affects scene understanding but is neither mapped nor excluded with reason
...
- blocked/manual-review/pending-reference usage rows are listed instead of omitted
- B/C record-only usage rows are listed when they matter for continuity or episode review
...
- blocked, manual-review, pending-reference, record-only, or B/C usage is hidden to make the table shorter
...
- `priority_level` and `production_handling` are separate fields in the machine table
- project summary reports counts by `priority_level` and `production_handling`
...
- priority, main-image task inclusion, and episode mapping are mixed into one ambiguous decision field
- `production_handling` is missing from global task rows or episode mapping rows
...
- number of global asset-state tasks by `task_type`, `priority_level`, and `production_handling`
