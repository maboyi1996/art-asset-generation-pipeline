...
- `usage_scene_ids`: the scene IDs where each candidate is used, if grouping is already clear
- `usage_evidence`: compact evidence explaining why the asset/state is present in this scene
- `no_new_asset_reason`
- `coverage_confidence`
- `coverage_notes`
...
- episode-local repeated minor characters or props that appear more than once within the same episode and therefore need consistency tracking
...
For characters and props only, a fully one-shot pass-by appearance can be excluded when it has no visual continuity need and no plot function. This exclusion does not apply to scenes/locations.

Each `episode_asset_usage_candidates` entry must be specific enough for later mapping:
- asset type
- raw asset/state name when known
- `episode_id`
- `scene_id`
- visible state clue, or `default/unchanged` when no state change is visible
- usage evidence

Do not invent final `asset_id`, `state_asset_id`, or `task_id` here. Those are assigned by Asset Registry after dedupe and state grouping.
...
Do not deliver this table as a separate final file by default. Its contents should feed the final `main-image-production-table.md`.
...
- every usage candidate has episode, scene, and usage evidence for later task mapping
