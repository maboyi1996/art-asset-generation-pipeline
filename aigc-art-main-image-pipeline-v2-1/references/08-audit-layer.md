...
- identify missing or conflicting global task fields
- identify global-task-to-episode-mapping gaps
...
For every episode, compare scene coverage against the final Episode Asset Usage Mapping Tables.
...
- an episode usage row lacks `state_asset_id` or `task_id`

This audit is separate from global task audit. Reused assets do not need duplicate prompts, but they must still appear in the episode mapping table with `task_type: reuse_existing` or a `reuse_from_task_id`.

## Global Task Map Audit

For every production-relevant state in the registry, check that the Global Asset State Task Map has:
- one stable `task_id`
- one explicit `task_type`
- `reference_task_id` and `reference_image` when `task_type` is `image_text_edit`
- `reuse_from_task_id` when `task_type` is `reuse_existing`
- `prompt` and `negative_prompt` when `task_type` is `text_to_image`
- `edit_instruction` and `state_delta_prompt` when `task_type` is `image_text_edit`
- `blocked` or `manual_review` status when required fields are missing

Flag `global_task_gap` if a state needs production handling but lacks a valid global task row.

Flag `task_duplication_risk` if the same `state_asset_id` appears as multiple independent generation tasks without a recorded reason.
...
- missing `production_handling`
- `priority_level` and `production_handling` collapsed into one ambiguous field
- S asset without `production_handling: main_task`
- A asset not handled as `main_task` without an explicit downgrade reason
- B asset with no destination: `deferred_task`, `record_only_mapping`, or `exclude_with_reason`
- C physical appearance that affects scene understanding but is neither mapped nor excluded with reason
- D item promoted beyond `exclude_with_reason` without source evidence
...
Prompt generation must not proceed for S/A main-task items with unresolved evidence gaps unless marked `待人工确认`.
...
## 7. Global Task Map Issues
| state_asset_id | task_id | issue | required_return_layer |

## 8. State Boundary Issues
| asset_id | invalid_state_or_prompt_scope | issue | required_return_layer |

## 9. Evidence / Conflict Issues
| asset_id | issue | affected_fields | recommendation |

## 10. Rating Issues
| asset_id | priority_level | production_handling | suggested_review | reason |

## 11. Manual Confirmation List
...
If global task map issue:
- return to Asset Registry if `state_asset_id`, `task_id`, `task_type`, or mapping fields are missing
- return to Character Identity Anchor if character reference identity fields are missing
- return to the relevant Main Image layer if prompt/edit fields are missing
...
- Global Asset State Task Map gaps are resolved or queued
- every episode mapping row can join to a global `task_id`
- invalid state splits are explicitly corrected, merged, or queued
- character prompt-scope leakage has been checked
- no S/A main-task prompt proceeds with hidden evidence gaps
