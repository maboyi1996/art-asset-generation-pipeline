...
This layer writes scene analysis and main image prompts for global asset-state tasks. It must use the PDF-verified scene templates.
...
This layer works from the Global Asset State Task Map. Do not create scene prompts directly from episode tables.

## Inputs

Use registry items with:
- `asset_type: scene`
- `task_type`: `text_to_image`, `reuse_existing`, `blocked`, or `manual_review`
- `production_handling: main_task` or `manual_review` for rows that need prompt work now
- derived `needs_main_image: true` unless the task is `reuse_existing` or `record_only`
...
- `used_in_episodes`
- `used_in_scenes`
- `reuse_from_task_id` when this scene state is reused
...
- each scene production row has `task_id` and `task_type`
- reused scene states are marked `reuse_existing` instead of regenerated
