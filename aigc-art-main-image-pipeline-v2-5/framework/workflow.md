...
flowchart TD
  ...
  AR --> REG["asset-registry.md<br/>输出: global-asset-registry<br/>global-task-draft<br/>episode-usage-map-draft"]
...
| 模块 | 顺序 | 作用 |
|---|---|---|
| 剧本解析与资产识别 | `script-intake -> scene-coverage -> asset-extraction -> state-variant -> asset-rating -> asset-registry` | 建立事实、候选资产、状态、评级、任务草稿和分集映射草稿。 |
...
| 剧本解析与资产识别 | `asset-registry.md` | `asset-candidate-list`、`state-variant-result`、`asset-rating-result`、`scene-coverage-table` | `任务类型字段审核规则.md`、`分集映射规则.md` | `global-asset-registry`、`global-task-draft`、`episode-usage-map-draft` |
...
| 资产生产与提示词生成 | `production-audit.md` | `global-asset-registry`、`global-task-draft`、`episode-usage-map-draft`、`character-anchor-result`、`scene-coverage-table` | `生产审计规则.md` | `production-audit-result` |
...
| 输出交付与质检 | `output-contracts.md` | `prompted-production-task-table`、`episode-usage-map-draft`、`production-audit-result` | `任务类型字段审核规则.md`、`分集映射规则.md`、`机器长表模板.md`、`人审短表模板.md` | `main-image-production-table.md`、`main-image-review-table.md` |
...
- `global-task-draft` 被 `character-anchor.md`、`production-audit.md`、`prompt-generation.md` 接住。
