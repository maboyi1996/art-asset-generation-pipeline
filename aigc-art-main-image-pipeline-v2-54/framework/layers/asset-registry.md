...
不重新抽取资产、不重新拆状态、不重新评级。本层可以生成任务草稿字段和可见排除记录，但不建立脸模锚点、不生成提示词正文。B/C 人物不需要项目级锚点，但可生产人物必须先进入 `global-task-draft` 形成 `text_to_image` 任务；泛称、群体或背景人物优先模板化、分组或可见排除，不得仅因重复出现制造独立单人脸模任务；人物候选去向台账只登记最终去向，不得替代全局生产任务。分集映射只引用全局任务草稿中的 `task_id`。
