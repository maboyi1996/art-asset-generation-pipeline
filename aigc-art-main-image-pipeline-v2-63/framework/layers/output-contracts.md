...
将三类评级前证据账本、全局注册、别名、S/A 状态、候选去向、模板覆盖、污染清洗、带提示词任务、分集映射和审计结果整理成最终两张 Markdown。
...
人审短表从机器长表派生，只保留人审需要的摘要，不得承载完整 ledger，不得减少、改名、合并、补建或重算机器长表任务。

本层不得直接输出 `prompt-manifest.json`，不得为迎合 JSON 改写任务、状态、评级、提示词或分集映射。JSON 只由 `json-manifest-generation.md` 在读取完成后的机器长表后生成。
