#!/usr/bin/env python3
"""Generate Seedance prompt-manifest.json from main-image-production-table.md.
...
DEFAULT_SKILL_ID = "aigc-art-main-image-pipeline-v2-63"
DEFAULT_SKILL_VERSION = "2.63"
...
def build_manifest(
    ...
    *,
    ...
    for row in task_rows:
        ...
        if not image_prompt:
            fail(f"Task {task_id} missing image prompt source")
        if not row.get("negative_prompt", ""):
            fail(f"Task {task_id} missing negative_prompt")
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate prompt-manifest.json from main-image-production-table.md")
    parser.add_argument("--production", required=True, type=Path, help="Path to main-image-production-table.md")
    parser.add_argument("--output", default="prompt-manifest.json", type=Path, help="Output JSON path")
