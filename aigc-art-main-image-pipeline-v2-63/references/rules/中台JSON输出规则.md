...
本文件只用于 `json-manifest-generation.md` 将完成后的 `main-image-production-table.md` 投影为 Seedance 元素库导入文件 `prompt-manifest.json`。

本规则解决的唯一问题是：

> 如何把 V2.63 机器长表中的已确认角色、场景、道具主图任务转换为中台可导入的 JSON 结构？

本文件不重新抽取资产、不重新评级、不重新拆状态、不补建任务、不重写提示词、不修复审计问题、不修改 Markdown。
...
| `source.skillId` | 固定为 `aigc-art-main-image-pipeline-v2-63`。 |
| `source.skillVersion` | 固定为 `2.63`。 |
...
| `prompt` 或 `edit_instruction` | `imagePrompt` | `text_to_image` 使用 `prompt`；`image_text_edit` 使用 `edit_instruction`，如同时有 `prompt` 可优先使用完整正向提示词。 |
| `negative_prompt` | `negativePrompt` | 直接映射。 |
...
`json-manifest-generation.md` 应优先调用 `scripts/generate_prompt_manifest.py` 执行确定性投影。脚本失败时，不得改由模型手工补造 JSON；必须返回对应机器长表或上游拥有层修复。

## 边界

- 不新增 JSON 合同之外的必填字段。
- 不输出旧版兼容结构作为主结构。
- 不把 evidence ledger、污染清洗、人物候选去向或人审短表整表塞进 JSON。
- 不把 `excluded` 任务放入 `assets[].appearance.appearances[]`。
- 不把 Markdown 表格字段名原样当成 JSON 字段。
- 不改变 `main-image-production-table.md` 或 `main-image-review-table.md` 的内容来迎合 JSON。
