...
```json
{
  ...
  "source": {
    "skillId": "aigc-art-main-image-pipeline-v2-0",
...
| `appearances[].imagePrompt` | Yes | Complete positive prompt for the main image or state image. |
| `appearances[].negativePrompt` | Recommended | Negative prompt. |
...
- Put all directly usable image prompts in `appearance.appearances[].imagePrompt`.
