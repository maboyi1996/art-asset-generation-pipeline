...
`artifact-manifest.json` 是 V3.1 项目输出文件夹的产物总索引。它声明每个文件的角色、路径、生产者、消费者和读取策略，让脚本能确定性找到输入输出。
...
| `skill_id` | string | 固定为 `aigc-art-main-image-pipeline-v3-1` |
| `skill_version` | string | 固定为 `3.1` |
